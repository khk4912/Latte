...
class LatteBot(commands.Bot):
    ...
    def settings_init(self):
        ...
        for config_filedir in server_config_files:
            with open(file=f'server_configs/{config_filedir}', mode='rt', encoding='utf-8') as config_file:
    ...
    def settings_save(self):
        ...
        try:
            with open(file=config_file_name, mode='wt', encoding='utf-8') as bot_config_file:
                print(f'token={token}')
                bot_config_file.write(f'token={token}')
        except Exception as e:
        ...
        for config in self.guild_configs:
            with open(file=f'./server_configs/{config["guild_name"]}_config.json', mode='wt', encoding='utf-8') as config_file:
...
bot = LatteBot(command_prefix=["라떼야 ", "라떼봇 ", "라떼 ", "라테야 ", "라테봇 ", "라테 "], description=desc)
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game(name="라떼는 달달합니다!", type=1))


@bot.event
async def on_guild_join(guild: discord.Guild):
    logger.info(f'새로운 서버에 참여했습니다! : {guild.name}')
    with open(file='./server_configs/sample_config.json', mode='rt', encoding='utf-8') as sample_config_file:
